...
for i in range(200):
    time.sleep(60)
    print("wait to start",i,"/ 100 minutes")
...
with tf.device(GPU):
    ...
    for net in nets:
        ...
        with tf.Session(config=config) as sess:
            ...
            if Gate is not None:
                ...
                confusion_matrix_experts = np.zeros((2, num_classes, num_classes))
                indices = np.arange(test_size)
                for b in range(0, test_size, batchsize):
                    batch_i = indices[b:b + batchsize]
                    batch_x = dataset.test_X[batch_i]
                ...
                with open(result_folder + net + "_confusion_matrix.txt", 'w') as file:
                    file.write("test size:" + str(test_size) + "\n")
                    file.write("count per label: \n")
                    file.writelines(matrix2rowstrs(np.sum(confusion_matrix_all, axis=1)[np.newaxis], "%i"))
                    ...
                    file.writelines(matrix2rowstrs(confusion_matrix_all_percent, "%i"))
    ...
    for net,color in zip(results.keys(),colors):
        ...
        for isTrain, history in acc.items():
            upper = max(upper, *history)
            lower = min(lower, history[-1])
    ...
    plt.axis([0,epochs-1,lower, upper])
