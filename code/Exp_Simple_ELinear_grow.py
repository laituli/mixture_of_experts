...
with tf.device(GPU):
    ...
    for num_experts in results.keys():
        acc = results[num_experts]['acc']

        vals["num_experts"].append(num_experts)
    ...
    plt.axis([min(vals["num_experts"]),max(vals["num_experts"]),lower, upper])
